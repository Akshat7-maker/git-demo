...
export default function BadgeInformation() {
    const { 
        register, 
        watch,
        formState: { errors } ,
        ...
        >
            <AccordionSummary expandIcon={<ChevronDown />}>
                ...
                >
                    ...
                    Badge Information {hasSectionError && "(Errors Found)"}
                </Typography>
            </AccordionSummary>
            <AccordionDetails>
                {/* 2. PASS PROPS EXPLICITLY */}
                {/* <ContentInput 
                    section="verifiedArtist" 
                    field="title" 
                    label="Section Title" 
                    max={65}
                    register={register}
                ...
                /> */}
                <TiptapEditor
                value = {watch("verifiedArtist.description")}
                onChange={(value: string) => setValue("verifiedArtist.description", value, { shouldValidate: true })}
                height={200}
                />
                {/* <ContentInput 
                    section="verifiedArtist" 
