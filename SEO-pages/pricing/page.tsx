...
import { ChevronDown, Save, FileText, AlertCircle, Verified, Eye, X, Globe } from "lucide-react";
...
import BrandsWeLove from "./components/badge";
import Affililate from "./components/freelance";
import JobBoard from "./components/badge";
import JobPostingPackage from "./components/freelance";
import BadgeInformation from "./components/badge";
import FreelanceSection from "./components/freelance";
import PricingPreview from "./components/Preview";
...
const badgeSchema = z.object({
  title: z.string().min(1, "Section title is required").max(65, "Title too long"),
  //   subText: z.string().min(1, "Sub-text is required"), // copied from artist hub
  description: z.string().min(1, "Description is required").max(375, "Description is required"),
});


const freelanceSchema = z.object({
  title: z.string().min(1, "Section title is required").max(70, "Title too long"),
  //   subText: z.string().min(1, "Sub-text is required"), // copied from artist hub
  description: z.string().min(1, "Description is required").max(800, "Description is required"),
});
...
export const landingPageSchema = z.object({
  artist: heroSchema,
  verifiedArtist: badgeSchema,
  freelanceArtist: freelanceSchema,
    ...
    <>
          ...
          <Box className="d-flex justify-content-between align-items-center mb-4">
            ...
            <Box display="flex" gap={2}>
              ...
              <Button
                ...
                disabled={isLoading}
                startIcon={
                  isLoading ? <CircularProgress size={20} /> : <Save size={18} />
                }
                sx={{
                  bgcolor: "#DBC489",
                  color: "#000",
      ...
      >
        ...
        >
          ...
          <PricingPreview data={methods.getValues()} />
        </Box>
      </Dialog>
    </>
  );
